...
//1
        ...
        if (!task3.isEmpty()) {
            users = repository.findBySurnameContainingOrderByName(task3);
        }
        if (!task4.isEmpty()) {
            users = repository.findAllId();
        }
        if (!task5.isEmpty()) {
            users = repository.findAllId2();
        }
        if (!task6.isEmpty()) {
            users = repository.findAllIdName();
        }
        if (!task7.isEmpty()) {
            users = repository.findByEmailNotContainingOrderByName(task7);
        }
        if (!task9.isEmpty()) {
            users = repository.findAllNew();
        }
        if (!task10.isEmpty()) {
            users = repository.findAllDist();
        }
          if (!task8.isEmpty()) {
